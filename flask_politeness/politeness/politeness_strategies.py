...
negativewords=[]
fin=open('./politeness/liu-negative-words.txt')
for line in fin:
...
positivewords=[]
fin=open('./politeness/liu-positive-words.txt')
for line in fin:
...
def loadData():
  ...
  wiki_requests = []  
  with open('./politeness/wikipedia.annotated.csv') as targetFile:
...
# Filter out unigrams with frequency < 10
  ...
  for request in binary_wiki_requests:
      ...
      request_count += 1
  return wiki_requests_data,wiki_requests_target,words    

def getAccuracy(binary_wiki_requests,wiki_requests_data,wiki_requests_target):
  ...
  return accuracy/len(binary_wiki_requests)
...
def preprocessing():
  ...
  return train(binary_wiki_requests)

def loadClassifier(wiki_requests_data,wiki_requests_target):
  clf = svm.SVC(probability=True)
  X_train = wiki_requests_data
  y_train = wiki_requests_target

  clf.fit(X_train,y_train)
  return clf

    
def get_proba(text,vocab,clf):
  parse = loads(server.parse(text))
  f = getPolitenessFeaturesFromParse(parse)
  text_words = getWordsFromParse(parse)
  f = sorted(f.iteritems())
    
  X_test = np.zeros(21+len(vocab))
  
  for i in xrange(21):
      X_test[len(vocab)+i] = f[i][1]
      if X_test[len(vocab)+i] == True:
          X_test[len(vocab)+i] = 1
      if X_test[len(vocab)+i] == False:
          X_test[len(vocab)+i] = 0
...
def getClassifierandVocab():
    if os.path.isfile('./wiki_clf.pkl') and os.path.isfile('./wiki_vocab.pkl'):
        with open('wiki_clf.pkl','rb') as f_clf:
            clf = pickle.load(f_clf)
        with open('wiki_vocab.pkl','rb') as f_vocab:
            vocab = pickle.load(f_vocab)
    
    else:
        wiki_requests_data, wiki_requests_target, vocab = preprocessing()
        clf = loadClassifier(wiki_requests_data, wiki_requests_target)

        with open('wiki_clf.pkl','wb') as f_clf:
            pickle.dump(clf,f_clf)
        with open('wiki_vocab.pkl','wb') as f_vocab:
            pickle.dump(vocab,f_f_vocab)
...
if __name__ == '__main__':
  ...
  #   #pickle.dump(wiki_requests_data,handle)
  #   wiki_requests_data = pickle.load(handle)
    
  # with open('target.pkl','rb') as handle:
  #   #pickle.dump(wiki_requests_target,handle)
  #   wiki_requests_target = pickle.load(handle)
  ...
  wiki_clf, wiki_vocab = getClassifierandVocab()
    
  text = 'Please help me out here. Hi, woops, I really appreciate your unlikely honesty.'
  print get_proba(text,wiki_vocab,wiki_clf)
